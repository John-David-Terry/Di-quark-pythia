#!/usr/bin/env python3
"""
Plot (1 / |ΔpT|) * (dN / d|ΔpT|) for
ΔpT = |pT_pi − pT_rem| using the cached
D_Rpi_ISRFSR_ON.npy array (ISR/FSR ON only).

Run from project root: python scripts/plots/plot_D_Rpi_jacobian_corrected.py
"""
import sys
...
def main():
    ...
    if D_vals.size == 0:
        print("No positive finite D_vals; nothing to plot.")
        return

    print(f"N raw D_vals: {D_vals.size}")
    print(f"min(D_vals) = {D_vals.min():.6e}")
    print(f"max(D_vals) = {D_vals.max():.6e}")
    print(f"mean(D_vals) = {D_vals.mean():.6e}")
    print(f"std(D_vals)  = {D_vals.std():.6e}")

    # 2) Histogram in magnitude over core
    xmax = np.quantile(D_vals, 0.995)
    bins = np.linspace(0.0, float(xmax), 100)
    counts, edges = np.histogram(D_vals, bins=bins, density=False)
    ...
    # Optionally drop the first bin if the center is too close to zero
    core_mask_plot = centers < np.quantile(D_vals, 0.8)
    ...
    if curve_fit is not None and centers_core.size >= 3:
        ...
        if fit_centers.size >= 3:
            ...
            sigma0 = float(np.std(D_vals)) or 0.5
            try:
                popt, _ = curve_fit(
                    half_gaussian,
                    fit_centers,
    ...
    ax.set_xlabel(r"$|\Delta p_T|$ [GeV]", fontsize=fontsize)
    ax.set_ylabel(r"$(1/R)\, dN/dR$ (arb. units)", fontsize=fontsize)
    ax.set_title(
        r"Jacobian-corrected $|\Delta p_T|$ distribution (ISR/FSR ON)", fontsize=fontsize
    )
